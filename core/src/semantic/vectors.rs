...
impl Artifact {
    pub fn valid(&self) -> bool {
        valid_hex(&self.token, 32)
            && valid_hex(&self.checksum, 64)
            && (1..=SHARD_CAPACITY).contains(&self.count)
            && (64..=MAX_FILE).contains(&self.bytes)
    }
}
...
impl Client {
    ...
    pub fn begin(
        ...
        if self.build.is_some()
            || !valid_hex(token, 32)
            || !(1..=2048).contains(&dimensions)
            || !(1..=SHARD_CAPACITY).contains(&count)
        {
        ...
        if !matches!(output, Output::Accepted { count: 0 }) {
    ...
    pub fn add(&mut self, vectors: &[Vector], cancel: &AtomicBool) -> Result<(), Failure> {
        ...
        if vectors.is_empty() || vectors.len() > 64 || build.added + vectors.len() > build.expected
        {
            return Err(Failure::InvalidInput);
        }
        let mut last = build.last;
        for vector in vectors {
            if vector.key <= last || !valid_vector(&vector.values, build.dimensions) {
                return Err(Failure::InvalidInput);
            }
            last = vector.key;
        }
        ...
        if !matches!(output,Output::Accepted {count} if count==expected) {
            return self.invalid_response();
        }
    ...
    pub fn finish(&mut self, cancel: &AtomicBool) -> Result<Artifact, Failure> {
        ...
        if !artifact.valid() || artifact.token != build.token || artifact.count != build.expected {
            return self.invalid_response();
        }
        Ok(artifact)
    }
    ...
    pub fn search(
        ...
        if self.build.is_some()
            || !valid_vector(vector, vector.len())
            || artifacts.len() > MAX_SHARDS
            || artifacts.iter().any(|artifact| !artifact.valid())
        ...
        if artifacts
            .iter()
            .any(|artifact| !tokens.insert(&artifact.token))
        {
            return Err(Failure::InvalidInput);
        }
        ...
        let Output::Found { hits, unavailable } = output else {
            return self.invalid_response();
        };
        if hits.len() > limit
            || hits.len()
                > artifacts
                    .iter()
                    .map(|artifact| artifact.count)
                    .sum::<usize>()
            || unavailable > artifacts.len()
            || (unavailable == artifacts.len() && !hits.is_empty())
            || hits.iter().any(|hit| {
                hit.key <= 0 || !hit.distance.is_finite() || !(0.0..=2.0).contains(&hit.distance)
            })
        {
            return self.invalid_response();
        }
        let mut keys = std::collections::HashSet::new();
        if hits.iter().any(|hit| !keys.insert(hit.key)) {
            return self.invalid_response();
        }
    ...
    fn invalid_response<T>(&mut self) -> Result<T, Failure> {
        self.close();
    ...
    fn request(
        ...
        let result = (|| {
            if self.connection.is_none() {
                self.connection = Some(Connection::spawn_in(&self.path, Some(&self.directory))?);
            }
            let response: Response = self
                .connection
                .as_mut()
                .ok_or(Failure::Unavailable)?
                .exchange(&bytes, deadline, cancel)?;
            if response.version != 1 || response.id != self.sequence {
                return Err(Failure::InvalidResponse);
            }
            match (response.result, response.error) {
                (Some(result), None) => Ok(result),
                (None, Some(WorkerError::Unavailable | WorkerError::Busy)) => {
...
"#
        ...
        fn client(&self) -> Client {
            Client::new(self.0.join("worker"), self.0.clone())
        }
    }
    ...
    fn build_protocol_validates_counts_and_reuses_worker() {
        ...
        let result = client
            .search(&[1.0, 0.0], &[artifact], 3, &AtomicBool::new(false))
            .unwrap();
    ...
    fn malformed_candidates_and_artifacts_close_the_connection() {
        for behavior in [
            ...
            let artifact = build(&mut client);
            assert_eq!(
                client.search(&[1.0, 0.0], &[artifact], 3, &AtomicBool::new(false)),
                Err(Failure::InvalidResponse),
                "{behavior}"
            );
    ...
    fn invalid_inputs_do_not_start_a_worker() {
        ...
        let cancel = AtomicBool::new(false);
        assert_eq!(
            client.begin("../outside", 2, 1, &cancel),
            Err(Failure::InvalidInput)
        );
        assert_eq!(
            client.begin(&"a".repeat(32), 2, SHARD_CAPACITY + 1, &cancel),
            Err(Failure::InvalidInput)
        );
        assert_eq!(
            client.search(&[0.0, 0.0], &[], 3, &cancel),
            Err(Failure::InvalidInput)
        );
        assert_eq!(
            client.search(&[1.0, 0.0], &[], 3, &AtomicBool::new(true)),
    ...
    fn query_cancellation_and_deadline_terminate_the_worker() {
        ...
        assert_eq!(
            client.search(&[1.0, 0.0], std::slice::from_ref(&artifact), 3, &cancel),
            Err(Failure::Cancelled)
        );
    ...
    fn native_round_trip_resolves_only_current_database_keys() {
        ...
        let documents: Vec<_> = ["/fixture/a.txt", "/fixture/b.txt"]
            .iter()
            .map(|path| Document {
                identity: path,
                path: Path::new(path),
        ...
        for ((id, revision), values) in identities.iter().zip([[1.0, 0.0], [0.0, 1.0]]) {
            store
                .put_embedding(*id, *revision, "fixture", &values)
                .unwrap();
        }
        let high = store.embedding_watermark().unwrap();
        let vectors = store
            .embedding_vectors(0, high, "fixture", 2, Arc::new(AtomicBool::new(false)))
            .unwrap()
            .vectors;
        ...
        client
            .begin(&"b".repeat(32), 2, 2, &AtomicBool::new(false))
            .unwrap();
        ...
        let result = client
            .search(&[1.0, 0.0], &[artifact], 2, &AtomicBool::new(false))
            .unwrap();
        let hits = store
            .resolve_embeddings(
                "fixture",
    ...
    fn native_builds_database_pages_and_cancelled_build_has_no_artifact() {
        use crate::{
            content::{ContentStore, Document},
            semantic::{
                Model,
                indexing::{ShardRange, build_shard, model_key},
            },
        };
        use std::path::Path;
        let fixture = Fixture::new("pass");
        ...
        assert_eq!(
            store
                .embedding_count(0, high, &key, 2, Arc::clone(&cancel))
                .unwrap(),
            130
        );
        assert!(
            store
                .embedding_count(0, 65_537, &key, 2, Arc::clone(&cancel))
                .is_err()
        );
        assert_eq!(
            store
                .embedding_count(0, high, "other", 2, Arc::clone(&cancel))
                .unwrap(),
            0
        );
        let path = PathBuf::from(std::env::var_os("BLINDSPOT_VECTOR_WORKER").expect("helper path"));
        let mut client = Client::new(path, fixture.0.clone());
        let mut range = ShardRange {
            token: "c".repeat(32),
            after: 0,
            through: high,
        };
        let result = build_shard(
            &store,
            &mut client,
            &model,
            &range,
        ...
        let found = client
            .search(&[1.0, 0.0], &[artifact], 10, &cancel)
            .unwrap();
        assert_eq!(found.unavailable, 0);
        assert_eq!(found.candidates.len(), 10);
        assert_eq!(
            store
                .resolve_embeddings(&key, 2, &found.candidates, Arc::clone(&cancel))
                .unwrap()
                .len(),
            10
        );
        ...
        assert!(
            build_shard(&store, &mut client, &model, &empty, cancel, |_| {})
                .unwrap()
                .is_none()
        );
