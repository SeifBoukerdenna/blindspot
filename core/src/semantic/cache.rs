...
use crate::{content::{Error, Result, vectors::{Shard, MAX_SHARDS}}, ffi::index_native::Directory};
use std::{collections::HashSet, ffi::OsStr, io::Read, os::unix::fs::MetadataExt,
    path::{Path, PathBuf}, sync::atomic::{AtomicBool, Ordering}};
...
pub fn available(database: &Path, shard: &Shard) -> Result<bool> {
    shard.validate()?;
    let Some(directory)=open(database)? else { return Ok(false); };
...
pub fn prune(database: &Path, retained: &[Shard], cancel: &AtomicBool) -> Result<u64> {
    check(cancel)?;
    if retained.len()>MAX_SHARDS { return Err(Error::Invalid("Vector cache capacity exceeded")); }
    for shard in retained { shard.validate()?; }
    let keep: HashSet<_>=retained.iter().map(|shard|shard.token.as_str()).collect();
    let Some(mut directory)=open(database)? else { return Ok(0); };
    let mut removed=0;
    let mut examined=0;
    loop {
        check(cancel)?;
        let Some(entry)=directory.next_entry()? else { return Ok(removed); };
        examined+=1;
        if examined>10_000 { return Err(Error::Invalid("Vector cache cleanup incomplete; retry")); }
        let Some(name)=entry.name.to_str() else { continue; };
        let Some(token)=name.strip_suffix(".ann") else { continue; };
        if token.len()!=32 || !token.bytes().all(|b|b.is_ascii_digit() || (b'a'..=b'f').contains(&b)) { continue; }
...
fn parent(database: &Path) -> Result<PathBuf> {
    if !database.is_absolute() || database.components().any(|part|matches!(part,std::path::Component::ParentDir)) {
        return Err(Error::Invalid("Invalid content database path"));
    }
    let parent=database.parent().ok_or(Error::Invalid("Invalid content database path"))?;
...
mod tests {
    ...
    fn cleanup_keeps_published_artifacts_unlinks_orphan_links_and_preserves_unrelated_files() {
        let root=std::env::temp_dir().join(format!("blindspot-vector-cache-{}",std::process::id()));
        ...
        let shard=Shard {after:0,through:1,model:"fixture".into(),dimensions:2,
            token:token().unwrap(),checksum:"b".repeat(64),count:1,bytes:64};
        ...
        assert!(matches!(prune(&database,&[],&AtomicBool::new(true)),Err(Error::Cancelled)));
        assert!(orphan.exists());
        assert_eq!(prune(&database,std::slice::from_ref(&shard),&AtomicBool::new(false)).unwrap(),2);
