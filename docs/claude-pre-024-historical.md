...
**M5 — Long tail.**
Calculator, clipboard history, converters, `:3000`, Quick Look. Each is independent; done in
the order they were actually missed. `tools.rs` holds the converters — epochs, bytes,
durations, lengths, weights, UUIDs, base64, URL, hex, `sha256`, `json` — each a detector
keyed on the *shape* of the query, never on natural language. **Snippets and window
management are still unbuilt**; window management is the one with a price, since moving
another app's windows needs Accessibility, which the hotkey deliberately avoided.
...
- **Six colourways** — Ember (warm dark, the default and the one it was drawn in),
  Graphite, Slate and Nocturne (dark), Parchment and Cool paper (light) — picked in the
  settings window and
...
**Quick Look is ⌘Y, not space.** The drawing asked for space and Finder does bind it, but
the panel's field always holds focus — that is the whole interface — so space is a space.
⌘Y is Finder's other binding for it and is free here.

**Quick Look takes key status, which dismisses the panel unless you stop it.** `Panel`
raises `previewing` *before* ordering the preview in, not after: `Preview.toggle` calls
`makeKeyAndOrderFront` before it returns, so a flag set on the way out arrives after
`windowDidResignKey` has already torn the panel down. Measured exactly that way round —
the panel vanished with the query still in it.

**`:3000` is `lsof`, so it cannot be synchronous.** `ports.rs` mirrors `files.rs` exactly —
supersede by generation, kill the child a new query made worthless, publish when done — so
there is one asynchronous pattern here rather than two. `lsof -F` field output is parsed
rather than its columns: a command name may contain spaces, which splits a column row in the
wrong place. `ps -o comm=` fills in each executable in one extra process, which is what gives
the row an icon and a working Reveal. ⌃↩ sends **SIGTERM**, never SIGKILL, and pid 0 and 1
are refused.

**`serde_json`'s `preserve_order` is on.** Without it `Value` is a `BTreeMap` and the `json`
tool silently sorts your keys — a formatter you cannot diff against the original. It costs no
new crates: indexmap, hashbrown and equivalent were already in the tree via `toml`, which is
the same criterion the redb decision was settled on.

**SHA-256 is hand-rolled**, like base64 above it. `sha2` brings five transitive crates for
sixty lines of fully specified arithmetic; the tests check it against the published vectors,
including a multi-block one.
...
**What you copy out of blindspot is recorded, and putting a clip back is not.** Pressing ↩
on a figure, a path, a pid or the model's answer goes through `ClipboardWatcher.copy`, which
lets the poller pick it up like any other copy — a history that omitted everything you
deliberately copied out of the launcher would have a hole in it, and re-copying something
already in history dedups to a bump. The one exception is putting an *image* clip back:
`writeObjects` offers several representations and its bytes are not the stored PNG, so the
hash differs and history would gain a near-duplicate. That one stays on `writeOwn`.

**The poll path must not touch the chrome.** `refresh(poll:)` is guarded on the text being
unchanged, so the mode, the prompt and the query's styling cannot have moved — and doing
them anyway forced a layout of the tab bar and a mutation of the field editor's storage
sixteen times a second while the model streamed. That was what made a generation flicker.
The entrance animation is likewise gated on the row *count* growing, not on the list
changing: measured over one generation, the old test fired four times and the new one twice,
so it was the smaller half of the problem — the chrome was the larger.
